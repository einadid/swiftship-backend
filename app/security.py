...
SECRET_KEY = os.getenv("JWT_SECRET", "swiftship-dev-secret-change-in-production")
ALGORITHM = "HS256"
...
def create_access_token(user_id: int) -> str:
    return _make_token({"sub": str(user_id)}, datetime.timedelta(minutes=ACCESS_TOKEN_MINUTES), "access")


def create_refresh_token(user_id: int) -> str:
    return _make_token({"sub": str(user_id)}, datetime.timedelta(days=REFRESH_TOKEN_DAYS), "refresh")

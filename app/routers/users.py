...
@router.get("/", response_model=schemas.UserPage, summary="List users with search + pagination (admin)")
def list_users(
    ...
    return schemas.UserPage(items=[schemas.UserOut.model_validate(u) for u in items], total=total, page=page,
                            page_size=page_size, total_pages=total_pages)


@router.get("/{user_id}", response_model=schemas.UserOut, summary="Get a single user (admin)")
...
@router.patch("/{user_id}/active", response_model=schemas.UserOut, summary="Block / unblock a user (admin)")
def set_active(user_id: int, body: schemas.UserActiveUpdate, db: Session = Depends(get_db),
...
@router.post("/{user_id}/reset-password", summary="Force-reset a user's password (admin)")
def reset_password(user_id: int, body: schemas.AdminSetPassword, db: Session = Depends(get_db),
                   _admin: models.User = Depends(get_current_admin)):
    user = db.get(models.User, user_id)
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
